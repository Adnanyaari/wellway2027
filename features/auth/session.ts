...
export async function getPrincipal(): Promise<Principal | null> {
  ...
  if (!process.env.DATABASE_URL || !(process.env.BETTER_AUTH_SECRET || process.env.AUTH_SECRET) || !requestHeaders.has("cookie")) return null;
  ...
  return { id: user.id, name: user.name, email: user.email,
    ...
    permissions: [...new Set(user.roles.flatMap(link =>
