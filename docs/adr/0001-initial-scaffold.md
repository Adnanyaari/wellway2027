...
The owner explicitly requested installation, an initial application/schema/auth foundation, and complete local checks. This supersedes the previous documentation-only phase and authorizes reversible scaffold implementation choices without another approval round. Full business features, production deployment, database migration/application, and fake content remain outside scope. An accepted production authentication decision is still required before exposing sign-in or other auth mutations.
...
- Better Auth with its Prisma adapter supplies session verification. No auth HTTP handler, password sign-in, OAuth, registration, recovery, bootstrap or session issuance exists in this scaffold. No custom password/session cryptography. Provider session records include sensitive bearer tokens and require strict DB access; review protected token storage, hashing, MFA, recovery and rate limits before activating authentication. Password fields are for provider hashes only, never plaintext. No credentials or accounts are created.
...
- `/dashboard` checks access then resolves to `/{locale}/dashboard`; both localized paths check access themselves. Arabic is the configurable initial default. `/ar` and `/en` are neutral unpublished shells. The sign-in route is a localized unavailable state, not a working form.
...
Rollback is removal/reversion of scaffold files; there is no database or deployment rollback because neither is changed. Affected documents: AGENTS, master, architecture, routes, database, auth, SEO, workflow, DevOps, and docs index. This is the first ADR; it supersedes none.
